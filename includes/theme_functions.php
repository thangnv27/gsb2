...
################################# VALIDATE SITE ################################
if($_SERVER['HTTP_HOST'] != 'noithatvanphongsme.com'){
...
/* GET THUMBNAIL URL */
function get_feedback_avatar_url() {
    $image_id = get_post_thumbnail_id();
    $image_url = wp_get_attachment_image_src($image_id, 'full');
    $image_url = $image_url[0];
    if($image_url != ""){
...
HTML;
    ...
    foreach ($cart as $p) :
        ...
        $desc = get_post_meta($product_id, "gioi_thieu", true);
        $masp = get_post_meta($product_id, "ma_sp", true);
        $purchases = intval(get_post_meta($product_id, "purchases", true));
        $image_url = $p['thumb'];
...
HTML;
        $counter++;
        update_post_meta($product_id, 'purchases', $purchases + 1);
    endforeach;
    ...
    $bill_html .= <<<HTML
                    <tr>
                        ...
                        <td style="text-align: right;">{$totalPrice} VNĐ</td>
                        <td></td>
                    </tr>
                    <tr>
                        <td colspan="6" style="text-align: right;text-transform: uppercase;font-weight: bold;">
                        ...
                        <td style="text-align: right;">{$discountFormat} VNĐ</td>
                        <td></td>
                    </tr>
                    <tr>
                        <td colspan="6" style="text-align: right;text-transform: uppercase;font-weight: bold;">
...
HTML;
    ...
    if($referrer and !empty($referrer)){
        ...
        wp_mail( $ref_email, $subject . " - For Sales", $bill_html, "", $attachment);
    }
